...
ChemStationBatchLoader::CHSDataVec ChemStationBatchLoader::getChemStationFiles(UIPlugin *plugin, const QDir &dir)
{
  CHSDataVec chsVec;
  ...
  while (dirIt.hasNext()) {
    const ChemStationFileLoader::Data chData = ChemStationFileLoader::loadHeader(plugin, dirIt.next());
...
QStringList ChemStationBatchLoader::getFilesList(UIPlugin *plugin, const QString &path, const Filter &filter)
{
  QDir dir(path);
  QStringList files;
  ...
  while (dirIt.hasNext())
    files.append(walkDirectory(plugin, dirIt.next(), filter));

  return files;
}

QStringList ChemStationBatchLoader::getFilesList(UIPlugin *plugin, const QStringList &dirPaths, const Filter &filter)
{
  QStringList files;
  ...
  for (const QString &path : dirPaths) {
    ...
    files.append(walkDirectory(plugin, path, filter));
  }

  return files;
}

ChemStationBatchLoader::CHSDataVec ChemStationBatchLoader::getCommonTypes(UIPlugin *plugin, const QString &path)
{
  QDir dir(path);
  ...
  while (dirIt.hasNext()) {
    ...
    if (innerDir.isReadable())
      chVecVec.push_back(getChemStationFiles(plugin, innerDir));
...
ChemStationBatchLoader::CHSDataVec ChemStationBatchLoader::getCommonTypes(UIPlugin *plugin, const QStringList &dirPaths)
{
  CHSDataVecVec chVecVec;
  CHSDataVec common;
  ...
  for (const QString &path : dirPaths) {
    ...
    chVecVec.push_back(getChemStationFiles(plugin, dir));
  }
...
QStringList ChemStationBatchLoader::walkDirectory(UIPlugin *plugin, const QString &path, const Filter &filter)
{
  QStringList files;
  ...
  while (dirIt.hasNext()) {
    ...
    ChemStationFileLoader::Data chData = ChemStationFileLoader::loadHeader(plugin, filePath);
